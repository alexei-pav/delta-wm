 ...
 *
 * The event handlers of deltawm are organized in an array which is accessed
...
setup(void)
{
	...
	XChangeProperty(dpy, wmcheckwin, netatom[NetWMName], utf8string, 8,
		PropModeReplace, (unsigned char *) "deltawm", 3);
...
spawn(const Arg *arg)
{
	...
	if (fork() == 0)
	{
		...
		die("deltawm: execvp '%s' failed:", ((char **)arg->v)[0]);
	}
}
...
xerror(Display *dpy, XErrorEvent *ee)
{
	...
	|| (ee->request_code == X_CopyArea && ee->error_code == BadDrawable))
		return 0;
	fprintf(stderr, "deltawm: fatal error: request code=%d, error code=%d\n",
...
{
	die("deltawm: another window manager is already running");
	return -1;
}
...
main(int argc, char *argv[])
{
	...
	if (!(dpy = XOpenDisplay(NULL)))
		die("deltawm: cannot open display");
	checkotherwm();
	setup();
#ifdef __OpenBSD__
